...
assertion_failure_proc :: proc(prefix, message: string, loc: runtime.Source_Code_Location) -> ! {
	...
	if err != nil {
		pc.printc_error("could not get backtrace for assertion failure: %v", err)
...
print :: proc( lines: []Line, padding := "    ", w: Maybe(io.Writer) = nil, no_temp_guard := false ) {
	...
	for line in lines {
		...
		table.row(tbl, padding, line.symbol, fmt.aprintf("%v%v%v",pc.INFO_COLOR," - ",pc.RESET), line.location)
	}
	color : string
	table.build(tbl, table.unicode_width_proc)
	for row in 0 ..< tbl.nr_rows {
		for col in 0 ..< tbl.nr_cols {
			table.set_cell_value(tbl,row,col,_intertal_colorize_table(table.get_cell(tbl, row, col).text))
...
@(private)
_intertal_colorize_table :: proc(text : string) -> string {
	res : bool
	cell_text := text
	cell_text = fmt.aprintf("%v%v%v",pc.NORMAL_COLOR,cell_text,pc.RESET)
	cell_text, res = strings.replace_all(cell_text,":",fmt.aprintf("%v%v%v",pc.ERR_COLOR,":",pc.RESET))
	return cell_text
}
